...
export function CreateHelperPage({
  ...
  function handleCreate() {
    try {
      ...
      if (prepared.participants !== undefined) {
        try {
          validateParticipantsValue(prepared.participants)
        } catch (error) {
          const message = error instanceof Error ? error.message : '参加枠の形式が不正です'
